...
struct CanController : public ICanController {
    ...
    Return<bool> downInterface(const hidl_string& name, const bool useIsotp) override;

  private:
    std::mutex mCanBusesGuard;
    std::map<std::string, sp<CanBus>> mCanBuses GUARDED_BY(mCanBusesGuard);
    std::mutex mCanIsotpBusesGuard;
    std::map<std::string, sp<CanBusIsotp>> mCanIsotpBuses GUARDED_BY(mCanIsotpBusesGuard);
};
