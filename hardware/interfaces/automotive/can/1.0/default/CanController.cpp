...
Return<ICanController::Result> CanController::upInterface(const ICanController::BusConfig& config) {
    ...
    if (config.useIsotp) {
        std::lock_guard<std::mutex> lck(mCanIsotpBusesGuard);
        if (mCanIsotpBuses.find(config.name) != mCanIsotpBuses.end()) {
            LOG(ERROR) << "ISOTP Bus " << config.name << " is already up";
    ...
    sp<CanBus> spBusService{nullptr};
    sp<CanBusIsotp> spIsotpBusService{nullptr};
    ...
    if (config.interfaceId.getDiscriminator() == IfIdDisc::socketcan) {
        ...
        if (config.useIsotp) {
            spIsotpBusService = new CanBusIsotp(ifaceName, config.bitrate, config.bitsp, config.dbitrate, config.dbitsp);
        } else {
            spBusService = new CanBusNative(ifaceName, config.bitrate, config.bitsp, config.dbitrate, config.dbitsp);
        }
    }
    // Virtual interface.
    else if (config.interfaceId.getDiscriminator() == IfIdDisc::virtualif) {
        spBusService = new CanBusVirtual(config.interfaceId.virtualif().ifname);
    }
    // SLCAN interface.
        ...
        spBusService = new CanBusSlcan(ttyName, config.bitrate);
    } else {
        return ICanController::Result::NOT_SUPPORTED;
    }

    if (spBusService != nullptr) {
        spBusService->setErrorCallback([this, name = config.name, useIsotp = config.useIsotp]() { downInterface(name, useIsotp); });

        const auto result = spBusService->up();
        if (result != ICanController::Result::OK) return result;

        if (spBusService->registerAsService(config.name) != OK) {
            LOG(ERROR) << "Failed to register ICanBus/" << config.name;
            if (!spBusService->down()) {
        ...
        std::lock_guard<std::mutex> lck(mCanBusesGuard);
        mCanBuses[config.name] = spBusService;
    }

    // setup ISOTP CAN bus
    if (spIsotpBusService != nullptr) {
        spIsotpBusService->setErrorCallback([this, name = config.name, useIsotp = config.useIsotp]() { downInterface(name, useIsotp); });

        const auto result = spIsotpBusService->up();
        if (result != ICanController::Result::OK) return result;

        if (spIsotpBusService->registerAsService(config.name) != OK) {
            LOG(ERROR) << "Failed to register ICanBusIsotp/" << config.name;
            if (!spIsotpBusService->down()) {
                LOG(WARNING) << "Failed to bring down CAN bus that failed to register";
            }
            return ICanController::Result::BAD_SERVICE_NAME;
        }
        ...
        std::lock_guard<std::mutex> lck(mCanIsotpBusesGuard);
        mCanIsotpBuses[config.name] = spIsotpBusService;
    }
...
static bool unregisterCanBusIsotpService(const hidl_string& name, sp<CanBusIsotp> busIsotpService) {
    auto manager = hidl::manager::V1_2::IServiceManager::getService();
    if (!manager) return false;
    const auto res = manager->tryUnregister(ICanBus::descriptor, name, busIsotpService);
    if (!res.isOk()) return false;
    return res;
}

Return<bool> CanController::downInterface(const hidl_string& name, const bool useIsotp) {
    LOG(VERBOSE) << "Attempting to bring interface down: " << name;

    auto success = true;
    if (useIsotp) {
        std::lock_guard<std::mutex> lck(mCanIsotpBusesGuard);
        auto busIsotpEntry = mCanIsotpBuses.extract(name);
        if (!busIsotpEntry) {
            LOG(WARNING) << "Interface " << name << " is not up";
            return false;
        }
        if (!unregisterCanBusIsotpService(name, busIsotpEntry.mapped())) {
            LOG(ERROR) << "Couldn't unregister " << name;
            // don't return yet, let's try to do best-effort cleanup
            success = false;
        }
        if (!busIsotpEntry.mapped()->down()) {
