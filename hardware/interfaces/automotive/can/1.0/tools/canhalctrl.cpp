...
static void usage() {
    ...
    std::cerr << "canhalctrl up <bus name> <type> <interface> [bitrate] [dbitrate]" << std::endl;
    ...
    std::cerr << " dbitrate - such as 2000000, 3000000" << std::endl;
    std::cerr << std::endl;
...
static int up(const std::string& busName, ICanController::InterfaceType type,
              const std::string& interface, bool useIsotp, uint32_t bitrate, uint32_t bsamplepoint,
              uint32_t dbitrate, uint32_t dbsamplepoint) {
    ...
    for (auto&& service : libcanhaltools::getControlServices()) {
        ...
        config.name = busName;
        config.useIsotp = useIsotp;
        config.bitrate = bitrate;
        config.bitsp = bsamplepoint;
        config.dbitrate = dbitrate;
        config.dbitsp = dbsamplepoint;
...
static int down(const std::string& busName, bool useIsotp) {
    for (auto&& service : libcanhaltools::getControlServices()) {
        auto ctrl = ICanController::getService(service);
        if (ctrl == nullptr) continue;

        if (ctrl->downInterface(busName, useIsotp)) return 0;
    }
...
int main(int argc, char* argv[]) {
    ...
    if (cmd == "up") {
        ...
        uint32_t bsamplepoint = 0;
        if (argc >= 5 && !android::base::ParseUint(argv[4], &bsamplepoint)) {
            std::cerr << "Invalid bsamplepoint!" << std::endl;
            usage();
            return -1;
        }
        uint32_t dbitrate = 0;
        if (argc >= 6 && !android::base::ParseUint(argv[5], &dbitrate)) {
            std::cerr << "Invalid dbitrate!" << std::endl;
            usage();
            return -1;
        }
        uint32_t dbsamplepoint = 0;
        if (argc >= 7 && !android::base::ParseUint(argv[6], &dbsamplepoint)) {
            std::cerr << "Invalid dbsamplepoint!" << std::endl;
            usage();
            return -1;
        }

        int retUpCan = up(busName, *type, interface, false, bitrate, bsamplepoint, dbitrate, dbsamplepoint);
        int retUpCanIsotp = up(busName, *type, interface, true /*useIsotp*/, bitrate, bsamplepoint, dbitrate, dbsamplepoint);
        return retUpCan + retUpCanIsotp;
    } else if (cmd == "down") {
        if (argc != 1) {
        ...
        int retDownCan = down(busName, false);
        int retDownCanIsotp = down(busName, true /*useIsotp*/);
        return retDownCan + retDownCanIsotp;
    } else {
...
int start_canhalctrl(int argc, char* argv[]) {
    if (argc < 1) return -1;
    return ::android::hardware::automotive::can::main(argc, argv);
}
